...
async function addMusic (req:Request, res:Response) {
    const { name, genresIds, youtubeLink }: insertSongType = req.body

    const { error } = postSongSchema.validate(req.body)
    if(error) return res.status(400).send({ error: error.details[0].message })

    try{
        const verifyIfExists: Array<responseSongType> = await findMusicByLink(youtubeLink)
...
async function getRandomSong (req:Request, res:Response) {
    try{
        const songs:Array<responseSongType> = await getAllSongs()
        const recommendation: responseSongType = await selectSong(songs)
...
async function getTopMusics (req:Request, res:Response) {
    ...
    try{
        const songs:Array<responseSongType> = await getAllSongsDescScore(amount)
        res.send(songs)
    }
    catch(e) {
        console.log(e)
